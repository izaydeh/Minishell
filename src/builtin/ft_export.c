 ...
 *     - Otherwise (e.g. "export var=" or "export var=value"), it adds or updates
...
void	ft_export(t_shell *test, char **args)
{
	...
	if (!args[1])
	{
		...
		for (j = 0; j < count; j++)
		{
			...
			if (equal_sign)
			{
				int name_len = equal_sign - env_copy[j];
	...
	else
	{
		...
		for (i = 1; args[i]; i++)
		{
			...
			var_name = get_name(new_var);
			{
				...
				while (test->env[j])
				{
					char *env_name = get_name(test->env[j]);
